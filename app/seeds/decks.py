import json
from app.models import db, Deck, Card, User_Card, User_Deck_Card, Trunk
from app.models.user_trunk_card import User_Trunk_Card
...
def seed_decks():
  ...
  demo_user_trunk = Trunk(
    user_id = demo_user_id
  )

  db.session.add(demo_user_trunk)
  db.session.commit()
  ...
  for card in starter_deck_data["water"]:
    ...
    if user_card_exists is None:
      ...
      user_card_exists.total_quantity += card["qty"]
  ...
  for user_card in User_Card.query.filter(User_Card.user_id == demo_user_id).all():
    new_user_trunk_card = User_Trunk_Card(
      user_card_id = user_card.id,
      trunk_id = demo_user_trunk.id,
      quantity = 0
    )
