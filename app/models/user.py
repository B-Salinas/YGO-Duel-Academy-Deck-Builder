...
class User(db.Model, UserMixin):
    ...
    profile_img_id = db.Column(db.Integer, db.ForeignKey(
        "profile_images.id"), nullable=False)
    title_id = db.Column(db.Integer, db.ForeignKey(
        "titles.id"), nullable=False)
    ...
    def sign_up(self):
        ...
        for card in starter_deck_json["fire"]:
            card_ref = Card.query.filter(
                Card.name.ilike(f"{card['name']}")).one()
            new_user_card = User_Trunk(
                user_id=self.id,
                card_id=card_ref.id,
